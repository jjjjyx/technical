...
<body>
	...
	<div class="container contanier-body"  >
		...
		<div class="row">
			...
			<form action="${path }/friend!list">
				<div class="panel panel-info">
					<div class="panel-heading"><ol class="breadcrumb" style="margin: 0px;">
						  <li><a href="${path }/person!index">首页</a></li>
						  <li><a href="${path }/friend!list">我的关注</a></li>
						</ol>
					</div>
					<div class="panel-body">
						<ul class="home_ul">
							<c:forEach var="l" items="${friendList.currentList }">
								<li>
							...
							<c:if test="${fn:length(friendList.currentList)==0 }">
								<li class="footer">
									<a href="${path }/person!index">还有没关注任何人，去首页逛逛吧</a>
								</li>
							</c:if>
						</ul>
					</div>
					<div class="panel-footer">
						<a href="javascript:void(0);"  data-currentpage="${friendList.pageutil.per }">上一页</a>
						<a href="javascript:void(0);"  data-currentpage="${friendList.pageutil.next }">下一页</a>
					</div>
				</div>
				<input type="hidden" name="currentPage" value="${friendList.pageutil.currentPage }">
				</form>
			</div>
			<!-- 右侧功能区 -->
		...
		$(document).ready(function() {
			...
			$(".icon-heart").click(function(){
				...
				jQuery.post(submitURL,form_data,function(jsonData){
					...
					if(info){
						alert("已取消关注");
						window.location.href="${path}/friend!list";
					}else{
						alert("啊哦，出了点小问题：原因：\n"+jsonData.text);
					}
				},"json");
